...
def calculate_overlap_intervals(peaks1, peaks2):
    """Calculate overlap metrics between two peak sets."""
    trees1 = {}
...
def test_peak_calling(run_gatac_only=False):
    ...
    if not run_gatac_only:
        ...
        log_path = os.path.join(os.path.dirname(__file__), "peak_calling.log")
        with open(log_path, 'w') as f:
            for result in results:
                print(result)
                f.write(result + '\n')

        # Assertions to verify peak calling quality
        assert jaccard > 0.95, f"Jaccard index too low: {jaccard:.3f} (expected > 0.95)"
        assert recall_snap > 0.96, f"SnapATAC2 recall too low: {recall_snap:.3f} (expected > 0.96)"
        assert recall_gatac > 0.98, f"GATAC recall too low: {recall_gatac:.3f} (expected > 0.98)"
    else:
        # Log GATAC-only results
        results = [
            f"GATAC:\t{gatac_time:.2f}s",
            f"Peaks called:\t{len(gatac_df):,}",
